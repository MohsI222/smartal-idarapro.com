...
// Find the main component function and its opening brace
let startLine = -1;
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('export function HrEnterpriseSuite')) {
    // Find the opening brace on this or next lines
    for (let j = i; j < Math.min(i + 5, lines.length); j++) {
      if (lines[j].includes('{')) {
        startLine = j;
        console.log(`Component body starts at line ${j + 1}`);
        console.log(`Line: ${lines[j].trim()}`);
        break;
      }
    }
    break;
...
// Count braces from the start of the component body
let openBraces = 0;
let inString = false;
