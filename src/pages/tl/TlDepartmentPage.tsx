...
export function TlDepartmentPage() {
  ...
  // Bypass module check if magic parameter is present
  const allowed = magicParam || isAdmin || approvedModules.includes("transport_logistics");
  ...
  const loadData = useCallback(async () => {
    // For magic links, use standalone endpoint without requiring token
    if (magicParam) {
      setLoading(true);
      try {
        const response = await fetch(`/api/tl/standalone-employees?token=${magicParam}`);
        ...
        if (isVehicle) {
          const v = await fetch(`/api/tl/standalone-vehicles?token=${magicParam}&department=${slug}`);
          const vData = await v.json();
          setVehicleLogs(Array.isArray(vData.logs) ? vData.logs : []);
        } else {
          const o = await fetch(`/api/tl/standalone-items?token=${magicParam}&department=${slug}`);
          const oData = await o.json();
          setOpsLogs(Array.isArray(oData.items) ? oData.items : []);
        }
      } catch {
    ...
    try {
      ...
      if (isVehicle) {
        ...
        setOpsLogs(Array.isArray(o.logs) ? o.logs : []);
      }
    } catch {
      toast.error(t("tl.loadErr"));
    } finally {
  ...
  }, [token, slug, allowed, isVehicle, t, magicParam]);
  ...
  useEffect(() => {
    // For magic links, resolve worker without requiring token
    if (magicParam && slug) {
      void fetch(`/api/tl/standalone-validate?token=${magicParam}`)
        .then((r) => r.json())
        .then((r) => {
          if (r?.success && r?.worker && r.worker.department === slug) setCtxWorker(r.worker);
        })
        .catch(() => {
          /* ignore */
        });
  ...
  const effectiveSender = useMemo(() => {
    ...
    return (workers?.length || 0) > 0 ? workers[0] : null;
