...
export function InventoryControlPanel({ t, products, onRefresh, onExport, onShowLowStock }: InventoryControlPanelProps) {
  ...
  const lowStockItems = safeProducts.filter(p => p.stock_pieces <= (p.low_stock_alert || 10) || p.stock_pieces <= 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        {/* Low Stock Alert */}
        <Card 
          className={`border-slate-800 bg-[#0a1628]/90 cursor-pointer transition-all hover:border-red-500/50 ${onShowLowStock && lowStockItems.length > 0 ? 'hover:bg-red-500/5' : ''}`}
          onClick={() => onShowLowStock && lowStockItems.length > 0 && onShowLowStock()}
        >
          <CardHeader className="border-b border-slate-800">
            <div className="flex items-center justify-between">
              <h3 className="font-black text-white">{t("inv.lowStockAlertTitle")}</h3>
              <div className="flex items-center gap-2">
                {lowStockItems.length > 0 && onShowLowStock && (
                  <span className="text-xs text-red-400">انقر للعرض</span>
                )}
