...
async function testScannerCleaned() {
  try {
    console.log('🧪 Testing Scanner with Cleaned Arabic Inventory...\n');
    
    const testUserId = 'c905d8d6-dcbd-4414-8214-7300ff6c7552';
    
    // Get cleaned inventory
    const inventoryResult = await pool.query('SELECT id, name, sku, unit_price, stock_pieces FROM inventory_products WHERE user_id = $1 ORDER BY name', [testUserId]);
    const inventory = inventoryResult.rows;
    
    console.log(`📋 Total inventory products: ${inventory.length}`);
    console.log('📋 Sample inventory products:');
    inventory.slice(0, 15).forEach((p, i) => {
...
السكر قالب 2 كيلو           5    15.00
الدقيق المميز 1 كيلو        4    12.00
المجموع                        143.50
`;
    ...
    for (const item of parsedItems) {
      ...
      for (const product of inventory) {
        ...
        if (productName.toLowerCase().includes(itemName.toLowerCase()) || 
            ...
            productName.includes(itemName.split(' ')[0]) || // Match first word
