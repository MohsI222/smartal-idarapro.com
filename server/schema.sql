...
CREATE INDEX IF NOT EXISTS idx_pos_agent_tokens_token ON pos_agent_tokens(token);
CREATE INDEX IF NOT EXISTS idx_pos_agent_tokens_user_id ON pos_agent_tokens(user_id);
CREATE INDEX IF NOT EXISTS idx_pos_agent_tokens_employee_id ON pos_agent_tokens(employee_id);
CREATE INDEX IF NOT EXISTS idx_pos_agent_tokens_is_active ON pos_agent_tokens(is_active);

-- POS Agent Sales Table
CREATE TABLE IF NOT EXISTS pos_agent_sales (
  id TEXT PRIMARY KEY,
  user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  employee_id TEXT,
  customer_name TEXT,
...
CREATE INDEX IF NOT EXISTS idx_pos_agent_sales_user_id ON pos_agent_sales(user_id);
CREATE INDEX IF NOT EXISTS idx_pos_agent_sales_employee_id ON pos_agent_sales(employee_id);

-- POS Agent Sales Lines Table
...
CREATE TABLE IF NOT EXISTS pos_agent_attendance (
  id TEXT PRIMARY KEY,
  user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  employee_id TEXT,
  clock_in_time TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  clock_out_time TIMESTAMPTZ,
  work_duration_minutes DOUBLE PRECISION
);

CREATE INDEX IF NOT EXISTS idx_pos_agent_attendance_user_id ON pos_agent_attendance(user_id);
CREATE INDEX IF NOT EXISTS idx_pos_agent_attendance_employee_id ON pos_agent_attendance(employee_id);
