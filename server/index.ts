...
// GET /api/inventory/standalone-products - Get products for standalone POS (public, token-based)
app.get("/api/inventory/standalone-products", async (req, res) => {
  try {
    ...
    // Validate token against pos_agent_tokens
    ...
    const rows = await db.prepare(
      `SELECT id, name, sku, unit_price, stock_pieces, unit_kind, pieces_per_carton, image_url
    ...
    console.error("[api/inventory/standalone-products GET] Error:", error);
    console.error("[api/inventory/standalone-products GET] Error details:", error instanceof Error ? error.message : String(error));
    res.status(500).json({ error: "فشل تحميل المنتجات", details: error instanceof Error ? error.message : String(error) });
  }
});
