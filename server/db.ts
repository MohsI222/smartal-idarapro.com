    ...
    initPromise = (async () => {
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        try {
          ...
          // Add image_url column to inventory_products if it doesn't exist
          try {
            await initPool.query(`
              ALTER TABLE public.inventory_products
              ADD COLUMN IF NOT EXISTS image_url TEXT
            `);
            console.log("[db] Added image_url column to inventory_products");
          } catch (alterErr) {
            console.warn("[db] Could not add image_url column to inventory_products:", alterErr instanceof Error ? alterErr.message : alterErr);
          }
